...
var EventFee = require('../model/EventFee');
...
describe('EventFee', function() {

    before(function(done) {
        const deletetables = Promise.coroutine(function*() {
            // runs before all tests in this block
            yield r.table("EventFee").delete();
        })();
        done();
    });
    describe('Test eventfee', function() {
        it('it should pass', function(done) {
            const tests = Promise.coroutine(function*() {
                ...
                console.log("delete eventfee 1");
                res = yield chai.request(server)
                    .post('/eventfee/delete')
                    .send({"id":aid1});
                logRes(res);
                res.should.have.status(200);
                res.body.should.be.a('object');
                ...
                console.log("get eventfee again");
                res = yield chai.request(server)
                    .get('/eventfees/'+id);
                logRes(res);
                res.should.have.status(200);
