...
var EventActivity = require('../model/EventActivity');
...
describe('EventActivity', function() {

    before(function(done) {
        const deletetables = Promise.coroutine(function*() {
            // runs before all tests in this block
            yield r.table("EventActivity").delete();
        })();
        done();
    });
    describe('Test eventactivity', function() {
        it('it should pass', function(done) {
            const tests = Promise.coroutine(function*() {
                ...
                console.log("add eventactivity 1");
                ...
                console.log("delete eventactivity 1");
                res = yield chai.request(server)
                    .post('/eventactivity/delete')
                    .send({"id":aid1});
                logRes(res);
                res.should.have.status(200);
                res.body.should.be.a('object');
                ...
                console.log("get eventactivity again");
                res = yield chai.request(server)
                    .get('/eventactivitys/'+id);
                logRes(res);
                res.should.have.status(200);
                res.body.should.be.a('array');
                res.body.length.should.be.eql(1);
