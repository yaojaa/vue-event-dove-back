...
var ConsumeLog = require('../model/ConsumeLog');
...
describe('ConsumeLog', function() {

    before(function(done) {
        const deletetables = Promise.coroutine(function*() {
            // runs before all tests in this block
            yield r.table("ConsumeLog").delete();
        })();
        done();
    });
    describe('Test consumelog', function() {
        it('it should pass', function(done) {
            const tests = Promise.coroutine(function*() {
                ...
                console.log("delete consumelog 1");
                res = yield chai.request(server)
                    .post('/consumelog/delete')
                    .send({"id":aid1});
                logRes(res);
                res.should.have.status(200);
                res.body.should.be.a('object');
                ...
                console.log("get consumelog again");
                res = yield chai.request(server)
                    .get('/consumelogs/'+id);
                logRes(res);
                res.should.have.status(200);
