...
var DefineSkin = require('../model/DefineSkin');
...
describe('DefineSkin', function() {

    before(function(done) {
        const deletetables = Promise.coroutine(function*() {
            // runs before all tests in this block
            yield r.table("DefineSkin").delete();
        })();
        done();
    });
    describe('Test defineskin', function() {
        it('it should pass', function(done) {
            const tests = Promise.coroutine(function*() {
                ...
                console.log("delete defineskin 1");
                res = yield chai.request(server)
                    .post('/defineskin/delete')
                    .send({"id":aid1});
                logRes(res);
                res.should.have.status(200);
                res.body.should.be.a('object');
                ...
                console.log("get defineskin again");
                res = yield chai.request(server)
                    .get('/defineskins/'+id);
                logRes(res);
                res.should.have.status(200);
