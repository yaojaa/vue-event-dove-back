...
var ResetPassword = require('../model/ResetPassword');
...
describe('ResetPassword', function() {
    ...
    before(function(done) {
        const deletetables = Promise.coroutine(function*() {
            ...
            yield r.table("ResetPassword").delete();
        })();
        done();
    });
    describe('Test resetpassword', function() {
        it('it should pass', function(done) {
            const tests = Promise.coroutine(function*() {
                ...
                console.log("add resetpassword 1");

                var res = yield chai.request(server)
                    .post('/resetpassword/add')
                    .send({
                        "id":id
                    });
                logRes(res);
                ...
                console.log("delete resetpassword 1");
                res = yield chai.request(server)
                    .post('/resetpassword/delete')
                    .send({"id":aid1});
                logRes(res);
                res.should.have.status(200);
                res.body.should.be.a('object');
                ...
                console.log("get resetpassword again");
                res = yield chai.request(server)
