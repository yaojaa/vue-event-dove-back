...
var ContactGroup = require('../model/ContactGroup');
...
describe('ContactGroup', function() {
    ...
    before(function(done) {
        const deletetables = Promise.coroutine(function*() {
            ...
            yield r.table("ContactGroup").delete();
        })();
        done();
    });
    describe('Test contactgroup', function() {
        it('it should pass', function(done) {
            const tests = Promise.coroutine(function*() {
                ...
                console.log("add contactgroup 1");

                var res = yield chai.request(server)
                    .post('/contactgroup/add')
                    .send({
                        "id":id
                    });
                logRes(res);
                ...
                console.log("delete contactgroup 1");
                res = yield chai.request(server)
                    .post('/contactgroup/delete')
                    .send({"id":aid1});
                logRes(res);
                res.should.have.status(200);
                res.body.should.be.a('object');
                ...
                console.log("get contactgroup again");
                res = yield chai.request(server)
