...
var SiteModule = require('../model/SiteModule');
...
describe('SiteModule', function() {

    before(function(done) {
        const deletetables = Promise.coroutine(function*() {
            // runs before all tests in this block
            yield r.table("SiteModule").delete();
        })();
        done();
    });
    describe('Test sitemodule', function() {
        it('it should pass', function(done) {
            const tests = Promise.coroutine(function*() {
                ...
                console.log("delete sitemodule 1");
                res = yield chai.request(server)
                    .post('/sitemodule/delete')
                    .send({"id":aid1});
                logRes(res);
                res.should.have.status(200);
                res.body.should.be.a('object');
                ...
                console.log("get sitemodule again");
                res = yield chai.request(server)
                    .get('/sitemodules/'+id);
                logRes(res);
                res.should.have.status(200);
