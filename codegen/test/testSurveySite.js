...
var SurveySite = require('../model/SurveySite');
...
describe('SurveySite', function() {

    before(function(done) {
        const deletetables = Promise.coroutine(function*() {
            // runs before all tests in this block
            yield r.table("SurveySite").delete();
        })();
        done();
    });
    describe('Test surveysite', function() {
        it('it should pass', function(done) {
            const tests = Promise.coroutine(function*() {
                ...
                console.log("delete surveysite 1");
                res = yield chai.request(server)
                    .post('/surveysite/delete')
                    .send({"id":aid1});
                logRes(res);
                res.should.have.status(200);
                res.body.should.be.a('object');
                ...
                console.log("get surveysite again");
                res = yield chai.request(server)
                    .get('/surveysites/'+id);
                logRes(res);
                res.should.have.status(200);
