...
var HotelOrder = require('../model/HotelOrder');
...
describe('HotelOrder', function() {

    before(function(done) {
        const deletetables = Promise.coroutine(function*() {
            // runs before all tests in this block
            yield r.table("HotelOrder").delete();
        })();
        done();
    });
    describe('Test hotelorder', function() {
        it('it should pass', function(done) {
            const tests = Promise.coroutine(function*() {
                ...
                console.log("delete hotelorder 1");
                res = yield chai.request(server)
                    .post('/hotelorder/delete')
                    .send({"id":aid1});
                logRes(res);
                res.should.have.status(200);
                res.body.should.be.a('object');
                ...
                console.log("get hotelorder again");
                res = yield chai.request(server)
                    .get('/hotelorders/'+id);
                logRes(res);
                res.should.have.status(200);
