...
var ModuleData = require('../model/ModuleData');
...
describe('ModuleData', function() {

    before(function(done) {
        const deletetables = Promise.coroutine(function*() {
            // runs before all tests in this block
            yield r.table("ModuleData").delete();
        })();
        done();
    });
    describe('Test moduledata', function() {
        it('it should pass', function(done) {
            const tests = Promise.coroutine(function*() {
                ...
                console.log("delete moduledata 1");
                res = yield chai.request(server)
                    .post('/moduledata/delete')
                    .send({"id":aid1});
                logRes(res);
                res.should.have.status(200);
                res.body.should.be.a('object');
                ...
                console.log("get moduledata again");
                res = yield chai.request(server)
                    .get('/moduledatas/'+id);
                logRes(res);
                res.should.have.status(200);
