...
var EventBadge = require('../model/EventBadge');
...
describe('EventBadge', function() {

    before(function(done) {
        const deletetables = Promise.coroutine(function*() {
            // runs before all tests in this block
            yield r.table("EventBadge").delete();
        })();
        done();
    });
    describe('Test eventbadge', function() {
        it('it should pass', function(done) {
            const tests = Promise.coroutine(function*() {
                ...
                console.log("delete eventbadge 1");
                res = yield chai.request(server)
                    .post('/eventbadge/delete')
                    .send({"id":aid1});
                logRes(res);
                res.should.have.status(200);
                res.body.should.be.a('object');
                ...
                console.log("get eventbadge again");
                res = yield chai.request(server)
                    .get('/eventbadges/'+id);
                logRes(res);
                res.should.have.status(200);
