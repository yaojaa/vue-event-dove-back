...
describe('<%= ModelName %>', function() {
    ...
    before(function(done) {
        const deletetables = Promise.coroutine(function*() {
            ...
            yield r.table("<%= ModelName %>").delete();
        })();
        done();
    });
    describe('Test <%= lowerModelName %>', function() {
        it('it should pass', function(done) {
            const tests = Promise.coroutine(function*() {
                ...
                console.log("add <%= lowerModelName %> 1");

                var res = yield chai.request(server)
                    .post('/<%= lowerModelName %>/add')
                    .send({
                        "id":id
                    });
                logRes(res);
                ...
                console.log("get <%= lowerModelName %>");
                res = yield chai.request(server)
                    .get('/<%= lowerModelName %>/'+id);
                logRes(res);
                res.should.have.status(200);
                ...
                console.log("delete <%= lowerModelName %> 1");
                res = yield chai.request(server)
                    .post('/<%= lowerModelName %>/delete')
                    .send({"id":aid1});
                logRes(res);
                res.should.have.status(200);
                res.body.should.be.a('object');
                ...
                console.log("get <%= lowerModelName %> again");
                res = yield chai.request(server)
                    .get('/<%= lowerModelName %>s/'+id);
                logRes(res);
                res.should.have.status(200);
                res.body.should.be.a('array');
                res.body.length.should.be.eql(1);
