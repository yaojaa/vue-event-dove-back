...
// 处理会员订单贝宝支付结果异步通知
exports.dealWithMemberOrderByPayPal = async function (result) {
    ...
    try {
        ...
        const orderNumber       = result.tracking_id;// 商户订单号
        const source            = result.sender_email;// 订单来源账户
        const destination       = result.seller_email;// 订单目标账户
        const paymentPlatformId = result['transaction[0].id'];// 支付平台流水号
        const total_fee         = Number(result['transaction[0].amount'].substring(4));// 订单总金额，单位为元
        const paymentTime       = result.payment_request_date;// 支出时间

        logger.debug('orderNumber = ', orderNumber, ' 处理会员订单贝宝支付结果异步通知被调用');
        ...
        if ((memberOrderInfo.totalPrice) !== total_fee) {
            ...
            logger.error('orderNumber = ', orderNumber, ' 贝宝返回的用户支付的会员订单金额是' + total_fee, '元');
            return false;
        }
        ...
        const channelFee         = amount.times(Big(fixParams.CHANNEL_FEE_TYPE.CHANNEL_FEE_TYPE_PAYPAL));// 第三方通道费
        ...
        const transaction        = {
            ...
            paymentMethod     : fixParams.PAYMENT_METHOD.CHANNEL_FEE_TYPE_PAYPAL,
            source            : source,
            ...
            currency          : fixParams.CURRENCY_NAME.DOLLAR,
        ...
        if (_.isEmpty(insertTransaction)) {
            logger.error('orderNumber = ', orderNumber, ' 贝宝支付后,插入事物失败了,待插入的数据信息是');
            logger.error(transaction);
            logger.error('orderNumber = ', orderNumber, ' 贝宝支付后,插入事物失败了,贝宝返回的原始信息是');
            logger.error(result);
            return false;
        }
        ...
        try {
            ...
            const updateMemberOrder = {
                payMethod     : fixParams.PAYMENT_METHOD.PAYMENT_METHOD_PAYPAL,
        ...
        logger.debug('orderNumber = ', orderNumber, ' 的会员订单,贝宝支付成功了!!!');

        // todo 发送相应的通知

        return true;
    } catch (err) {
        logger.error('dealWithMemberOrderByPayPal ', err);
        return false;
    }

};

// 处理短信或者邮件订单PayPal支付结果异步通知
exports.dealPayPalSmsEmailOrderNotify = async function (result, req) {
    try {
        ...
        logger.debug('orderNumber = ', orderNumber, ' 处理门票订单PayPal支付结果异步通知被调用');
        ...
        if ((orderInfo.totalPrice) !== total_fee) {
            ...
            logger.error('orderNumber = ', orderNumber, ' PayPal支付返回的用户支付的短信邮件充值订单金额是' + total_fee, '元');
            return false;
        }
        ...
        const transaction        = {
            ...
            paymentMethod     : fixParams.PAYMENT_METHOD.PAYMENT_METHOD_PAYPAL,// 支付方式
        ...
        const insertTransaction  = await Transaction.addTransaction(transaction);
        if (_.isEmpty(insertTransaction)) {
            logger.error('orderNumber = ', orderNumber, ' PayPal支付后,插入事物失败了,待插入的数据信息是');
            logger.error(transaction);
            logger.error('orderNumber = ', orderNumber, ' PayPal支付后,插入事物失败了,PayPal返回的原始信息是');
            logger.error(result);
            return false;
        }

        // 短信邮件订单后续处理逻辑,更改订单状态status为paid,更改订单支付方式paymentMethod为paypal,等其他操作
        const updateOrder = {
            status       : fixParams.ORDER_STATUS.ORDER_STATUS_PAID,// 更改订单状态status为paid
            paymentMethod: fixParams.PAYMENT_METHOD.PAYMENT_METHOD_PAYPAL,// 更改订单支付方式paymentMethod为paypal
        };
        Notice.updateSmsEmailOrderById(orderInfo.id, updateOrder);

        logger.debug('orderNumber = ', orderNumber, ',orderId = ', orderInfo.id, ' 的 transactionType = ', transactionType, ' 短信邮件充值订单,PayPal支付成功了!!!');

        logger.debug('orderNumber = ', orderNumber, " 贝宝支付短信邮件充值成功，发送相关通知");
        sysNotice.sendSmsEmailNoticeAfterOrderPay(req, orderInfo.id);

        return true;
    } catch (err) {
        logger.error('dealPayPalSmsEmailOrderNotify ', err);
        return false;
    }
};

// 处理门票订单PayPal支付结果异步通知
exports.dealPayPalTicketOrderNotify = async function (result, req) {
    try {
        ...
        const eventInfo = await Event.getEventById(orderInfo.eventId, ['userId']);
        ...
        if ((orderInfo.totalPrice) !== total_fee) {
            ...
            logger.error('orderNumber = ', orderNumber, ' PayPal返回的用户支付的门票订单金额是' + total_fee, '元');
            return false;
        }
        ...
        const channelFee         = amount.times(Big(fixParams.CHANNEL_FEE_TYPE.CHANNEL_FEE_TYPE_PAYPAL));// 支付通道费
        ...
        // 门票订单后续处理逻辑,更改订单状态status为paid,更改订单支付方式paymentMethod为paypal,等其他操作
        ...
        logger.debug('orderNumber = ', orderNumber, ',orderId = ', orderInfo.id, ' 的订单PayPal支付成功了!!!');

        // 发送通知
        logger.debug('orderNumber = ', orderNumber, "门票贝宝支付成功，发送邮件相关通知");
        sysNotice.sendNoticeAfterOrderPay(req, orderInfo.id);

        return true;
    } catch (err) {
        logger.error('dealPayPalTicketOrderNotify ', err);
        return false;
    }
};
